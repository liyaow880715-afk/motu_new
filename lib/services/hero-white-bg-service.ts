...
export async function findExistingWhiteBg(productName: string, sourceImageUrl: string) {
...
export async function generateWhiteBgImage(
  ...
  // Check cache first
  const existing = await findExistingWhiteBg(productName, sourceImageUrl);
  if (existing) {
    return existing.imageUrl;
  }

  const referenceImages = await resolveReferenceImage(sourceImageUrl);
  ...
  const storageDir = join(env.STORAGE_ROOT ?? "./storage", "hero-scene", "white-bg");
...
export async function getWhiteBgImagesByProduct(productName: string) {
